...
define( [ "N/file", "N/log", "N/record", "N/runtime" ],
function ( file, log, record, runtime ) {
////////////////////////////////////////////////////
...
RUNTIME_CONTEXT.getRemainingUsage = function() {
  return 1000;
}
const ABAI_TABLES = {
...
function MainScript() {
  debug('');
  debug(JSON.stringify(RUNTIME_CONTEXT));
  audit('MainScript', '======START======');
  var batchId = RUNTIME_CONTEXT.getParameter({ name: 'custscript_abai_batch_log_id' });
  var abaiBatchRecord = record.load({type: 'customrecord_abai_int_batch_logger', id: batchId});
  debug(JSON.stringify(abaiBatchRecord));
  processFile( // INSTITUTION
    abaiBatchRecord.getValue({fieldId: 'custrecord_institue_fileid'}),
    ABAI_TABLES.INSTITUTION, parseInstitution, ABAI_INSTITUTION
  );
  // processFile(  // INSTITUTION ADDRESS
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_institueaddr_fileid'}),
  //   ABAI_TABLES.INSTITUTION_ADDRESS, parseInstitutionAddress, null
  // );
  // processFile( // COORDINATOR
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_coordinaor_fileid'}),
  //   ABAI_TABLES.COORDINATOR, parseCoordinator, ABAI_COORDINATOR
  // );
  // processFile( // COURSE SEQUENCE
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_coursesseq_fileid'}),
  //   ABAI_TABLES.COURSE_SEQUENCE, parseCourseSequence, ABAI_COURSE_SEQUENCE
  // );
  // processFile( // COURSE
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_course_fileid'}),
  //   ABAI_TABLES.COURSE, parseCourse, ABAI_COURSE
  // );
  // processFile( // AP WAIVER
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_apwaiver_fileid'}),
  //   ABAI_TABLES.APWAIVER, parseApWaiver, null
  // );
  // processFile( // INSTRUCTOR GROUP
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_instructorgrp_fileid'}),
  //   ABAI_TABLES.INSTRUCTOR_GROUP, parseInstructor, null
  // );
  // processFile( // COURSE SEQUENCE ASSIGNMENT
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_courseseq_crsass_fileid'}),
  //   ABAI_TABLES.COURSE_ASSIGNEMNT, parseAssignment, ABAI_ASSIGNMENT
  // );
  // processFile( // ALT COURSE ID
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_alt_courseid_fileid'}),
  //   ABAI_TABLES.ALT_COURSE_ID, parseAltCourseId, null
  // );
  // processFile( // CONTENT HOURS
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_cont_hours_fileid'}),
  //   ABAI_TABLES.CONTENT_HOURS, parseContentHours, ABAI_CONTENT_HOURS
  // );
  // processFile( // ALLOCATION HOURS
  //   abaiBatchRecord.getValue({fieldId: 'custrecord_cont_hsallocat_fileid'}),
    ...
    for ( var line = 1; line < fileContent.length; line++ ) {
      ...
      try {
        ...
        if ( errorCount > 5 ) {
          ...
          debug('NetSuite Usage: ' + RUNTIME_CONTEXT.getRemainingUsage());
...
function createRecord(type, data, parseFunction) {
  var abaiRecord = record.create({type: type, isDynamic: true});
