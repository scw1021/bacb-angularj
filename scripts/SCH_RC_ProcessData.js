...
const RUNTIME_CONTEXT = nlapiGetContext();
...
function processAbaiToRcRecord(
  ...
  var abaiSection = abaiResultSet.getResults(startIndex,endIndex);
  ...
  while ( abaiSection.length > 0 ) {
    ...
    for ( var abaiIndex in abaiSection ) {
      ...
      var rcID = searchFunction(abaiSection[abaiIndex].getText('internalid'), rcData);
      ...
      for ( var rcField in rcFieldsToSet ) {
        if ( rcFieldsToSet.hasOwnProperty(rcField) ) {
          try {
            objRecord.setFieldValue(rcField, rcFieldsToSet[rcField]);
          }
          catch (ex) {
    ...
    startIndex = endIndex;
    endIndex += 1000;
    abaiSection = abaiResultSet.getResults(startIndex,endIndex);//.getRange({start: startIndex, end: endIndex});
  }
}
 ...
 */
function executeSavedSearch(searchName, strRecordType) {
  ...
  try {
    ...
    savedSearch = nlapiLoadSearch(strRecordType, RUNTIME_CONTEXT.getSetting('SCRIPT', searchName)).runSearch();;
  }
  catch (ex) {
...
function loadTableData(nsTable, storage, process) {
  ...
  var nsSearch = nlapiSearchRecord(nsTable.Title, null, null, searchColumns);
  ...
  var searchRessult = nsSearch.getResults(startIndex, endIndex);
  ...
  while (searchResult.length > 0) {
    ...
    searchResult = nsSearch.getResults(startIndex, endIndex);// nsSearch.getRange({ start: startIndex, end: endIndex });
  }
...
function toDictionary(storage, value) {
  ...
  storage[value.getFieldValue('internalid')] = value;
}
...
function toXrefDictionary(storage, value) {
  ...
  storage.InternalId[value.getFieldValue('internalid')] = value;
  storage.CourseSequence[value.getFieldValue('custrecord_crse_seq_crse_asign_crse')] = value;
}
...
function translateAbaiToRcId(abaiSearchId, rcSearchTable, rcSearchValue) {
  ...
  for ( var searchIndex in rcSearchTable ) {
    if ( abaiSearchId === rcSearchTable[searchIndex].getFieldValue(rcSearchValue) ) {
...
function translateInstitution(abaiSearchId, rcData) {
  ...
  return result ? result.getFieldValue('custrecord_uniq_instn_name') : miss(RC_TABLES.INSTITUTION);
}
...
function translateDepartment(abaiSearchId, rcData) {
  ...
  return result ? result.getFieldValue('custrecord_course_sequence_department') : miss(RC_TABLES.DEPARTMENT);
}
function translateCourse(abaiSearchId, rcData) {
  var result = RC_COURSE[abaiSearchId];
  return result ? result.getFieldValue('internalid') : miss(RC_TABLES.COURSE);
}
...
function getRcInstitutionData(abaiData) {
  ...
  function InstitutionName() {
    var instName = abaiData.getFieldValue('custrecord_insttn_name');
    var uniqueName = abaiData.getFieldValue('custrecord_uniq_instn_name');
    return {
      'custrecord_rc_inst_name': uniqueName ? uniqueName : instName,
    }
  }
  return ABAI_MAPPED_RC_INSTITUTIONS[abaiData.getFieldValue('internalid')] = Object.assign( {
    'custrecord_rc_instn_rm_id': abaiData.getFieldValue('internalid'),
    'custrecord_rc_inst_credit_system': abaiData.getFieldValue('custrecord_bacb_insti_cred_sys'),
    'custrecord_rc_inst_accrediting_body': abaiData.getFieldValue('custrecord_bacb_insti_accr_body'),
    'custrecord_rc_inst_website': abaiData.getFieldValue('custrecord_institn_website'),
    }, InstitutionName()
  )
}
function getRcInstitutionAddressData(abaiData) {
  function getInstitution() {
    return  ABAI_INSTITUTIONS[abaiData.getFieldValue('custrecord_instn_addr_inst_id')].getFieldValue( 'custrecord_uniq_instn_name' );
  }
  return Object.assign({
    'externalid': abaiData.getFieldValue('externalid'),
    'custrecord_rc_inst_add_inst': getInstitution(),
    'custrecord_rc_inst_addr_crm_id': abaiData.getFieldValue('custrecord_institutn_addr_crm_id'),
    'custrecord_rc_inst_add_address1': abaiData.getFieldValue('custrecord_instn_addr_addr_line1'),
    'custrecord_rc_inst_add_address2': abaiData.getFieldValue('custrecord_instn_addr_addr_line2'),
    'custrecord_rc_inst_add_city': abaiData.getFieldValue('custrecord_instn_addr_city'),
    'custrecord_rc_inst_add_state': abaiData.getFieldValue('custrecord_instn_addr_state_provnc'),
    'custrecord_rc_inst_add_country': abaiData.getFieldValue('custrecord_instn_addr_country'),
    'custrecord_rc_inst_add_postal_code': abaiData.getFieldValue('custrecord_instn_addr_postal_code'),
  })
}


function getRcDepartmentData(abaiData) {
  return RC_DEPARTMENTS[abaiData.getFieldValue('internalid')] = {
    'custrecord_rc_depart_name': abaiData.getFieldValue('custrecord_rc_depart_name'),
    // 'custrecord_rc_depart_website': '',
    'custrecord_rc_dept_institution_name': RC_INSTITUTIONS[abaiData.getFieldValue('custrecord_rc_depart_institution')].getFieldValue('custrecord_rc_inst_name'),
    'custrecord_rc_depart_institution': RC_INSTITUTIONS[abaiData.getFieldValue('custrecord_rc_depart_institution')]
  }
}
...
function getRcCourseData(abaiData) {
  function getDeptAndInst(abaiData) {
    ...
    var abaiInstitutionId = abaiData.getFieldValue('custrecord_course_institutn_id');
    var rcDepartmentId = ABAI_SEQUENCE[ABAI_XREF.CourseSequence[
      abaiData.getFieldValue('internalid')] // get the Course Sequence ID
      .getFieldValue('custrecord_crse_seq_crse_asign_crseseqnc') // Get the Unique Department ID
    ]
    return {
      'custrecord_rc_course_institution': ABAI_MAPPED_RC_INSTITUTIONS[abaiInstitutionId].getFieldValue('custrecord_rc_inst_name'),
      'custrecord_rc_course_inst': ABAI_MAPPED_RC_INSTITUTIONS[abaiInstitutionId].getFieldValue('internalid'),
      'custrecord_rc_course_department': RC_DEPARTMENTS[rcDepartmentId].getFieldValue('custrecord_rc_depart_name'),
      'custrecord_rc_course_dept': rcDepartmentId
    }
  }
  return RC_COURSE[abaiData.getFieldValue('internalid')] = Object.assign({
    // Set all normal values
    'custrecord_rc_crse_crm_id': abaiData.getFieldValue('custrecord_crse_seqnc_crm_identifier'),
    'externalid': abaiData.getFieldValue('custrecord_crse_seqnc_crm_identifier'),
    'custrecord_rc_course_title': abaiData.getFieldValue('custrecord_coursE_title'),
    'custrecord_rc_course_number': abaiData.getFieldValue('custrecord_course_number'),
    'custrecord_rc_course_credit_level': abaiData.getFieldValue('custrecord_course_credit_level'),
    'custrecord_rc_course_credit_hours': abaiData.getFieldValue('custrecord_course_credit_hours'),
    'custrecord_rc_course_instruction_mode': abaiData.getFieldValue('custrecord_course_mode_of_instrctn')},
...
function getRcContentHoursData(abaiData) {
  ...
  return RC_CONTENT_HOURS[abaiData.getFieldValue('internalid')] = Object.assign({
    'custrecord_rc_cnthrs_crm_id': abaiData.getFieldValue('custrecord_content_hrs_crm_identifier'),
    'custrecord_rc_content_hr_start_date': checkNull(abaiData.getFieldValue('custrecord_cnt_hrs_activ_strt_date')),
    'custrecord_rc_content_hr_end_date': checkNull(abaiData.getFieldValue('custrecord_cnt_hrs_activ_end_date')),
    'custrecord_rc_content_hr_start_year': yearSubstring(abaiData.getFieldValue('custrecord_cnt_hrs_activ_strt_year')),
    'custrecord_rc_content_hr_end_year': yearSubstring(abaiData.getFieldValue('custrecord_cnt_hrs_activ_end_year')),
    'custrecord_rc_content_hr_start_qtr': abaiData.getFieldValue('custrecord_cnt_hrs_activ_strt_quar'),
    'custrecord_rc_content_hr_end_qtr': abaiData.getFieldValue('custrecord_cnt_hrs_activ_end_quar'),
    'custrecord_rc_content_hr_start_sem': abaiData.getFieldValue('custrecord_cnt_hrs_activ_strt_sem'),
    'custrecord_rc_content_hr_end_sem': abaiData.getFieldValue('custrecord_cnt_hrs_activ_end_sem'),
    'custrecord_rc_content_hr_edition': abaiData.getFieldValue(''),
    ...
    'custrecord_rc_content_hr_course': RC_COURSE[ABAI_XREF.InternalId[
      abaiData.getFieldValue('custrecord_cnt_hrs_crseseq_crseassignmnt' )]].getFieldValue('internalid'),
  })
}
