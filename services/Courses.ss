...
function _Create(objRequest, objResponse) {
    E$.logAudit('CREATE Courses', '=====START=====');
    try {
    ...
    if(httpBody){
      ...
      nlapiLogExecution('ERROR', 'Create Course_BROKE', 'Body could not be read my dude');
    }
    var objDataResponse = {
      Response: 'F',
...
function _Delete(objRequest, objResponse) {
    E$.logAudit('DELETE Courses', '=====START=====');
    ...
    try {
        ...
        nlapiLogExecution('ERROR', 'DELETE Courses', 'The call to get the current web session failed.:' + ex.message)
    }
	...
	if (stBody) {
        ...
        nlapiLogExecution('ERROR', 'DELETE Courses', 'Body of the request is not defined.');
    }
...
function _Read (objRequest, objResponse) {
   ...
   try {
        ...
        nlapiLogExecution('ERROR', 'READ Courses', 'The call to get the current web session failed.:' + ex.message)
    }
	...
	if (stBody) {
        ...
        nlapiLogExecution('ERROR', 'READ Courses', 'Body of the request is not defined.');
    }
...
function _Update(objRequest, objResponse) {
    E$.logAudit('UPDATE Courses', '=====START=====');
    try {
      var objSession = nlapiGetWebContainer().getShoppingSession();
      var stCustId = objSession.getCustomer().getFieldValues().internalid;
  }
  catch (ex) {
      nlapiLogExecution('ERROR', 'Update Course', 'The call to get the current web session failed.:' + ex.message)
  }
