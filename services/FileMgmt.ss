...
function service (objRequest, objResponse) {
  nlapiLogExecution('DEBUG', 'FILEMGMT param', objRequest.getParameter('param'))
  // nlapiLogExecution('DEBUG', 'FILEMGMT headers', objRequest.getAllHeaders())
  nlapiLogExecution('DEBUG', 'FILEMGMT body', objRequest.getBody())
...
function _Egnyte(objRequest, objResponse) {
  ...
  nlapiLogExecution('AUDIT', "Write File", "======END======");
	objResponse.setContentType('PLAINTEXT');
	objResponse.write(JSON.stringify(urlResponse));
}

function _Read(objRequest, objResponse) {
  // E$.logAudit("READ File", "=====START=====");
  try {
    var objSession = nlapiGetWebContainer().getShoppingSession();
    var stCustId = objSession.getCustomer().getFieldValues().internalid;
  } catch (ex) {
  ...
  // E$.logAudit("READ File", "======END======");
	objResponse.setContentType('PLAINTEXT');
	objResponse.write(JSON.stringify(objDataResponse));
};

function _ReadDocTypes(objRequest, objResponse) {
  E$.logAudit("READ Doc Types", "=====START=====");
  var objDataResponse = {
    Response: "F",
    Message: ""
  };
