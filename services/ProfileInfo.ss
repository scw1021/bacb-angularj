...
function Create(objRequest, objResponse) {

};

function Delete(objRequest, objResponse) {

};

function Read(objRequest, objResponse) {
	E$.logAudit('Read Personal Information', '=====START=====');
    var stCustId = 0;
	//var stCustId = nlapiGetContext().getUser();
	//if (!stCustId) {
	//	throw nlapiCreateError('INVALID_SESSION', 'Client session is invalid.');
    //}

	var objRxData = {};
	var stBody = objRequest.getBody();
	E$.logDebug('Read Personal Information', 'stBody: ' + stBody);
	if (stBody) {
        objRxData = JSON.parse(stBody);
        stCustId = objRxData.CustomerId;
    }
    else {
        throw nlapiCreateError('INVALID_DATA', 'Body of returned object is undefined.')
    };
    ...
    E$.logAudit('Read Personal Information', '======END======');
...
function _Update(objRequest, objResponse) {
    E$.logAudit('Update Personal Information', '=====START=====');
    var stCustId = 0;
	//var stCustId = nlapiGetContext().getUser();
	//if (!stCustId) {
	//	throw nlapiCreateError('INVALID_SESSION', 'Client session is invalid.');
	...
	var objRxData = {};
	var stBody = objRequest.getBody();
	E$.logDebug('Update Personal Information', 'stBody: ' + stBody);
	if (stBody) {
        objRxData = JSON.parse(stBody);
        stCustId = objRxData.CustomerId;
    }
    ...
    E$.logAudit('Update Personal Information', '======END======');
	objResponse.setContentType('PLAINTEXT');
	objResponse.write(JSON.stringify(objDataResponse));
};
...
function AltUpdate(objRequest, objResponse) {
  E$.logAudit('Alt Update Personal Information', '=====START=====');
  var stCustId = 0;
  // var stCustId = nlapiGetContext().getUser();
  // if (!stCustId) {
  //   throw nlapiCreateError('INVALID_SESSION', 'Client session is invalid.');
  ...
  var objRxData = {};
  var stBody = objRequest.getBody();
  E$.logDebug('Alt Update Personal Information', 'stBody: ' + stBody);
  if (stBody) {
      objRxData = JSON.parse(stBody);
      stCustId = objRxData.CustomerId;
  }
  ...
  E$.logAudit('Alt Update Personal Information', '======END======');
  objResponse.setContentType('PLAINTEXT');
  objResponse.write(JSON.stringify(objDataResponse));
};
